...
public class TypeCasting01 {
	...
	public static void main(String[] args) {
		...
		//다운 캐스팅 : 실행 시 오류 발행, t3에는 TigerChild는내포하고 있지 않기때문에 
			//다운캐스팅 실행시 오류 발생( Animal과 Tiger 만 내포중 )
		System.out.println("==== instanceof 출려 ====");
		...
		Tiger t5=null; 		//블락 밖에서 캐스팅할 변수 선언
		if (a3 instanceof Tiger ) {	//true일때 다운 캐스팅 해라
			t5 =(Tiger) a3;
		...
		//3. a3 TigerChild 로 다운캐스팅
		TigerChild t6 = null;	//블락 밖에서 변수 선언
		if (t3 instanceof TigerChild) {
			t6 = (TigerChild) t3;
		}
		//t6 : TigerChild 타입 (Animal, Tiger, TigerChild 의 필드와 메소드를 모두 접근 가능)
		t6.age = 2;	//Animal
		t6.tigerSound = "어흥어흥"; 	//Tiger
		t6.tigerChildSound = "어흥";	//TigerChild
		
		//t6 (TigerChild) ---> Animal
		Animal t7 = (Animal) t6;	//t7 : Aniaml, Tiger, TigerChild 내포하고 있고 Animal 타입
