...
public class LogicalOperator {
	...
	public static void main(String[] args) {
		...
		System.out.println(false ^ true); // true
		System.out.println(false ^ false); // false
		System.out.println(false ^ (5<2)); // false ^ fasle(5는 2보다 크니 false)
		System.out.println(( 10 > 20 ) ^ ( 30 > 20 ) ); // false ^ true = true
		System.out.println("===============================");
		...
		System.out.println(false || ! ( 5 < 3 )); //false || ture = true
		
		//2. 비트연산자 논리연산이 가능 하다. <<== <<쇼트 서킷을 발생하지 않도록 처리>>
		//	AND ( & ), OR ( | ), XOR ( ^  ), NOT ( ~ ) 
		System.out.println(true & true); // true
		System.out.println(true | false); //true
		...
		//3. 쇼트 서킷 논리연산자에서 앞의 내용으로 뒤의 값의 유무없이 식별 가능 하면 뒤의 값은 처리 하지 않는다.
		
		System.out.println(true && true); // 쇼트 서킷이 발생 되지 않는다. (true가 앞에 올 경우), 뒤의 값을 반드시 확인
		System.out.println(false && true);  //쇼트 서킷이 발생 (앞에서 false)
		//AND 연산자에서는 처음 값이 false 가  오게되면 뒤의 값이 어떤 값이 오더라도 false
		
		System.out.println(false || true); //쇼트 서킷이 발생 되지 않는다.
		System.out.println(true || true); //쇼트 서킷이 발생됨
		//OR 연산에서 처음 값이 ture 이면 쇼트 서킷이 발생됨
		System.out.println("=============AND 연산에서 숏트 서킷 발생 및 방지======================");
		int value1 = 3;
		System.out.println(false && ++value1 > 6); // 쇼트 서킷 이 발생됨 (논리연산자 : && )
		System.out.println(value1); // 쇼트서킷으로인해 ++이 실행이 되지 않아 3으로 출력
		...
		// XOR ( ^ ) : 논리연산, 비트연산 <== 뒤의 값을 반드시 확인해야 결과를 알 수 있으므로 숏트서킷이 발생되지 않는다.
