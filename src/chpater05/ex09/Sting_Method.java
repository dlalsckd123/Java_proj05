...
public class Sting_Method {
	...
	public static void main(String[] args) {
		...
		System.out.println(str1.length()); //글자의 갯수 : 11자
		System.out.println(str2.length()); // 13자 
		
		//2. 문자열 검색 (charAt(), indexOf(), lastIndexOf() )
		System.out.println(str1.charAt(1)); //Hello java -> H는 0번방 , //1번 방의 문자를 출력 : e
		...
		System.out.println(str1.lastIndexOf('a')); // indexOf와 반대로 뒤에서부터 시작해서 첫번째 나오는 방을 카운트 :9
		...
		//String str3 = 2.3; //요류발생 : 2.3 (double 형 데이터를 String에 저장시 오류)
		...
		System.out.println(Arrays.toString(arr1)); //아스키코드값으로 나옴, 72 101 108 108 111 32 74 97 118 97 33
		...
		System.out.println(str13.substring(0,5));  //0번에서 시작해서 4번 방 앞까지 잘림
			//모든 프로그램에서 시작은 포함, 끝은 미포함
		System.out.println(str13.substring(3, 5));
		
		//split() : 특정 문자를 기준으로 잘라서 문자열 배열에 저장
		String[] strArray = "abc/def-ghi jkl".split("/|-| "); //특수문자 / or - 를 잘라버려라!
		...
		//6. 문자열의 내용 비교(equals(), equalsTgnoreCase())
		
			//equlas() : Heap의 값을 비교, 대문자 소문자 까지 체크
		...
		System.out.println(str21.equalsIgnoreCase(str22)); //true , equals로 하면 대소문자가 맞지 않아 false
