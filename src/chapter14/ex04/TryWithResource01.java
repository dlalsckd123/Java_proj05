...
public class TryWithResource01 {
	...
	public static void main(String[] args) {
		// Try 구문에서 리소스 자동해제, 객체를 사용후에 반드시 그 객체를 제거해서 메모리를 최적화
		...
		//콘솔을 사용해서 문자열을 입력 받도록 하는 클래스
		InputStreamReader ir1 = null;

		//일반예외 (Catched Exception) : 프로그램 작성시 컴파일 단계에서 오류
			// 1. 예외를 남에게 전가	: throws 를 사용해서 호출하는데 쪼개서 처리하도록 미루는 방법
		...
		try {
			...
			// 
			System.out.println("IOExcetpion 이 발생 되었습니다. 하나의 문자만 넣어주세요.");
			e.printStackTrace();	//
		}finally {
			//일반적으로 객체를 사용후 fially 블락에서 객체를 제거함.
